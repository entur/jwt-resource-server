...
public class CorsAPIMethodsTest {
    ...
    private final List<String> allowedMethods = Arrays.asList("GET", "POST", "OPTIONS");
    private final List<String> forbiddenMethods = Arrays.asList("PUT", "DELETE");

    private final List<String> hosts = Arrays.asList("https://petstore.swagger.io", "https://myportal.apigee.io", "https://developer.entur.org");

    @Test
    public void cors_is_allowed() {
        hosts.forEach(host -> allowedMethods.forEach(method ->
                given().header("Origin", host).header("Access-Control-Request-Method", method)
                        .when().log().all()
    ...
    public void cors_is_forbidden() {
        hosts.forEach(host -> forbiddenMethods.forEach(method ->
                given().header("Origin", host).header("Access-Control-Request-Method", method)
                        .when().log().all()
                        .options("http://localhost:" + port + "/unprotected")
                        .then().log().all()
