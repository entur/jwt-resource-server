...
public class ReflectionAuthorizationService {
    ...
    public ReflectionAuthorizationService(RoleAssignmentExtractor roleAssignmentExtractor, boolean authorizationEnabled, OrganisationChecker organisationChecker, AdministrativeZoneChecker administrativeZoneChecker,
            EntityResolver entityResolver, Map<String, List<String>> fieldMappings) {
    ...
    public boolean isAuthorized(String requiredRole, Collection<?> entities) {
        ...
        List<RoleAssignment> relevantRoles = roleAssignmentExtractor.getRoleAssignmentsForUser().stream().filter(roleAssignment -> requiredRole.equals(roleAssignment.getRole())).collect(toList());

        for (Object entity : entities) {
            boolean allowed = entity == null || relevantRoles.stream()
                    // Only one of the role assignments needs to match for the given entity and
                    // required role
                    .anyMatch(roleAssignment -> authorized(roleAssignment, entity, requiredRole));
            if (!allowed) {
                // No need to loop further, if not authorized with required role for one of the
    ...
    public Set<String> getRelevantRolesForEntity(Object entity) {
        return roleAssignmentExtractor.getRoleAssignmentsForUser().stream()
                .filter(roleAssignment -> roleAssignment.getEntityClassifications().get(ENTITY_TYPE).stream()
                        .anyMatch(entityTypeString -> entityTypeString.equalsIgnoreCase(entity.getClass().getSimpleName().toLowerCase()) || entityTypeString.contains(ENTITY_CLASSIFIER_ALL_TYPES)))
                .map(RoleAssignment::getRole).collect(Collectors.toSet());
    }
    ...
    private boolean checkEntityClassifications(String entityTypename, Object entity, RoleAssignment roleAssignment, String requiredRole) {
        ...
        if (!containsEntityTypeOrAll(roleAssignment, entityTypename)) {
            logger.debug("No match for entity type {} for required role {}. Role assignment: {}", entity.getClass().getSimpleName(), requiredRole, roleAssignment);
            return false;
        }
    ...
    private boolean checkEntityClassification(String entityType, Object entity, List<String> classificationsForEntityType) {
        ...
        if (mappings != null) {
            ...
            if (isBlacklist) {
                // If the list is detected to be blacklist values, every mapped field must
                // match.
                return mappings.stream().allMatch(mappedField -> isAllowedForFieldAndClassification(mappedField, entity, classificationsForEntityType, true));
            } else {
                // If the list is detected to be a white list, any match is enough.
                return mappings.stream().anyMatch(mappedField -> isAllowedForFieldAndClassification(mappedField, entity, classificationsForEntityType, false));
            }
        } else {
    ...
    private boolean isAllowedForFieldAndClassification(String entityType, Object entity, List<String> classificationsForEntityType, boolean isBlacklist) {
        ...
        if (isBlacklist) {
            ...
            isAllowed = classificationsForEntityType.stream().noneMatch(classification -> removeUnderscore(classification.substring(1)).equalsIgnoreCase(stringValue));

        } else {
            isAllowed = classificationsForEntityType.stream().anyMatch(classification -> removeUnderscore(classification).equalsIgnoreCase(stringValue));
    ...
    private Optional<Field> findFieldFromClassifier(String classifier, Object entity) {
        return Stream.of(entity.getClass().getDeclaredFields()).filter(field -> classifier.equalsIgnoreCase(field.getName())).findFirst();
    }
    ...
    public boolean checkAdministrativeZone(RoleAssignment roleAssignment, Object entity) {
        return roleAssignment.getAdministrativeZone() == null || roleAssignment.getAdministrativeZone().isEmpty() || administrativeZoneChecker.entityMatchesAdministrativeZone(roleAssignment, entity);
    }
