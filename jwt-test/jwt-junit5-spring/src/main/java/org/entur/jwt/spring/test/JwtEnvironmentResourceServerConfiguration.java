    ...
    public String getProperty(String id, String propertyName) {
        if (id == null || id.isEmpty()) {
            ...
            if (environment instanceof ConfigurableEnvironment) {
                ...
                StreamSupport.stream(propertySources.spliterator(), false).filter(ps -> ps instanceof EnumerablePropertySource).map(ps -> ((EnumerablePropertySource<?>) ps).getPropertyNames()).flatMap(Arrays::<String>stream)
                        .filter(name -> name.startsWith(prefix)).filter(name -> name.endsWith("." + propertyName)).forEach(propName -> props.put(propName, environment.getProperty(propName, String.class)));
                if (props.size() == 1) {
